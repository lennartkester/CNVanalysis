...
## specify directory in which cram files are located ##
## directory should contain tumorCramList.csv and normalCramList.csv which list the cram files in corresponding order ##
## specify panel of normals hdf5 file ##
## if possible provide a file which contains the tumors and a file which contains the normals in matching order ##
## set the #SBATCH --array variable to the number of pairs in the analysis

tumor=$(cat tumorCramList.csv | sed -n ${SLURM_ARRAY_TASK_ID}p)
normal=$(cat normalCramList.csv | sed -n ${SLURM_ARRAY_TASK_ID}p)
...
interval_list='MedExome_hg38_capture_targets_padded250.interval_list'
temp=$(echo $normal | awk '{split($0,a,"_") ; print a[1] "*.vcf.gz" }' | head -1 )
